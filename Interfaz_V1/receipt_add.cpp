...
void __fastcall TReceiptAdd::Button1Click(TObject *Sender)
{
    ...
    if(Application->MessageBoxA("Está seguro que quiere agregar la receta?","Confirmación de carga",MB_YESNO)== IDYES)
    {
        if(value == 2)
        {
        ...
        {
            Application->MessageBoxA("El nombre elegido ya està en uso","Error");
        }
        else if(value == 1)
        {
